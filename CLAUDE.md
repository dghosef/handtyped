...
It works by:
1. **Hardware-gating input** — only accepting keystrokes from the built-in MacBook keyboard via the IOHIDManager driver (Transport = "SPI" or "FIFO"). External USB keyboards, Bluetooth keyboards, and software-injected keystrokes (osascript, CGEventPost) are all blocked.
...
- The app only runs on Apple Silicon Macs (SPI/FIFO transport = built-in keyboard). Intel Mac support is not planned.
