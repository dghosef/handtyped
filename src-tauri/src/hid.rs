...
extern "C" {
    ...
    fn CFStringGetCString(s: *const c_void, buf: *mut c_char, buf_size: isize, encoding: u32) -> bool;
...
fn hid_log(msg: &str) {
    let line = format!("[hid] {}\n", msg);
    eprint!("{}", line);
    if let Ok(mut f) = std::fs::OpenOptions::new().create(true).append(true).open("/tmp/humanproof-hid.log") {
        use std::io::Write;
...
unsafe extern "C" fn hid_input_callback(
    ...
    let suspicious = if int_val != 0 {
        ...
        if is_modifier {
            ...
            let last_ns = ctx.state.last_keydown_ns.load(std::sync::atomic::Ordering::Relaxed);
            let suspicious = last_ns > 0 && wall_ns.saturating_sub(last_ns) < SUSPICIOUS_INTERVAL_NS;
            ctx.state.last_keydown_ns.store(wall_ns, std::sync::atomic::Ordering::Relaxed);
            suspicious
        }
    } else {
        false
    ...
    if int_val != 0 {
        ctx.state.pending_builtin_keydowns
            .fetch_add(1, std::sync::atomic::Ordering::SeqCst);
    }

    let event = KeyEvent { t: wall_ns, kind: kind.into(), key: usage, flags: 0, suspicious };
...
unsafe fn device_property_string(device: IOHIDDeviceRef, key: &str) -> Option<String> {
    ...
    let mut buf = [0i8; 256];
    if CFStringGetCString(val, buf.as_mut_ptr(), buf.len() as isize, K_CF_STRING_ENCODING_UTF8) {
...
unsafe fn device_property_u32(device: IOHIDDeviceRef, key: &str) -> Option<u32> {
    ...
    if CFNumberGetValue(val, K_CF_NUMBER_SINT32_TYPE, &mut result as *mut i32 as *mut c_void) {
        Some(result as u32)
    } else {
        None
    }
...
unsafe extern "C" fn hid_device_matched_callback(
    ...
    if context.is_null() || device.is_null() { return; }
    let ctx = &*(context as *const CallbackContext);

    let transport = device_property_string(device, "Transport")
        .unwrap_or_else(|| "unknown".into());
    let product_id = device_property_u32(device, "ProductID").unwrap_or(0);
    let vendor_id = device_property_u32(device, "VendorID")
        .unwrap_or(K_APPLE_VENDOR_ID as u32);
    ...
    if let Ok(mut ki) = ctx.state.keyboard_info.lock() {
        *ki = Some(KeyboardInfo { vendor_id, product_id, transport });
    }
}
...
pub unsafe fn request_input_monitoring_access() {
    ...
    hid_log(&format!("IOHIDCheckAccess: {} (0=granted,1=denied,2=unknown)", access));
...
pub fn start_hid_capture(state: Arc<AppState>) {
    unsafe {
        ...
        if ret == 0 {
            state.hid_active.store(true, std::sync::atomic::Ordering::Release);
            hid_log("capture active");
            return;
        }

        hid_log(&format!("IOHIDManagerOpen failed: {}, will retry from background", ret));
        ...
        .spawn(move || {
            loop {
                ...
                unsafe {
                    ...
                    if a != K_IOHID_ACCESS_TYPE_GRANTED {
                        continue;
                    }
                    ...
                    if manager.is_null() { continue; }

                    IOHIDManagerSetDeviceMatchingMultiple(manager, matching_array.as_concrete_TypeRef());

                    let ctx_ptr = leak_callback_context(
                        Arc::clone(&state),
                        start_mach,
                        start_wall_ns,
                        tb,
