...
pub fn build_and_sign(
    ...
    let mut zip_buf: Vec<u8> = Vec::new();
    {
        ...
        zip.start_file(format!("{prefix}document.txt"), opts)
            .map_err(|e| e.to_string())?;
        zip.write_all(&txt_bytes).map_err(|e| e.to_string())?;

        zip.start_file(format!("{prefix}document.rtf"), opts)
            .map_err(|e| e.to_string())?;
        zip.write_all(&rtf_bytes).map_err(|e| e.to_string())?;

        zip.start_file(format!("{prefix}keystroke-log.jsonl"), opts)
            .map_err(|e| e.to_string())?;
        zip.write_all(&log_bytes).map_err(|e| e.to_string())?;

        zip.start_file(format!("{prefix}session-meta.json"), opts)
            .map_err(|e| e.to_string())?;
        zip.write_all(&meta_bytes).map_err(|e| e.to_string())?;

        zip.start_file(format!("{prefix}bundle.sig"), opts)
            .map_err(|e| e.to_string())?;
        zip.write_all(sig_content.as_bytes())
            .map_err(|e| e.to_string())?;
